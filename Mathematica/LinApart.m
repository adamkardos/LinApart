...
Begin["Private`"]
$LinApartPATH = DirectoryName[$InputFileName] <> "/";

Get[$LinApartPATH<>"tools_general.m"]
Get[$LinApartPATH<>"tools_parallel.m"]

Get[$LinApartPATH<>"tools_univariate_nonlinear.m"]
Get[$LinApartPATH<>"tools_multivariate_linear.m"]
Get[$LinApartPATH<>"tools_residue_method.m"]
Get[$LinApartPATH<>"tools_Leinartas_method.m"]
Get[$LinApartPATH<>"tools_Groebner_method.m"]

Get[$LinApartPATH<>"preprocessor.m"]
Get[$LinApartPATH<>"partial_fraction_algorithms.m"]
...
ClearAll[$LinApartPathologicalAtoms]
...
LinApart[expr_, varOrVars_, options : OptionsPattern[]] :=
    expr /; !FreeQ[expr, $LinApartPathologicalAtoms]
...
LinApart[expr_, var_, options:OptionsPattern[]] := (
    Message[LinApart::infiniteInput, var];
    $Failed
) /;!FreeQ[expr, Alternatives@@$LinApartInfinities]||!FreeQ[var, Alternatives@@$LinApartInfinities]
...
LinApart[expr_, var_Symbol, options:OptionsPattern[]] := (
    Message[LinApart::protectedVar, var];
    $Failed
) /; MemberQ[Attributes[var], Protected]
...
LinApart[expr_, vars_List, options:OptionsPattern[]] := (
    Message[LinApart::protectedVar, Select[vars, MemberQ[Attributes[#], Protected] &]];
    $Failed
) /; AnyTrue[vars, MemberQ[Attributes[#], Protected] &]
