...
* coefficient matrix, entry (5,82).  Runs the cover-first cache branch.
*
* INPUT SOURCE (the rational function below is entry (5,82) of the
...
*
* ENTRY STRUCTURE: 13 distinct linear denominators, maximum
* multiplicity 3 (benchmark power index sum_m = 24).
*
...
*   determinant branch : 7.92 s  (Phase 1 4.63 s, Phase 2 2.63 s)
*   cover-first cache   : 6.23 s  (Phase 1 2.04 s, Phase 2 3.41 s)
...
*   tform -w16 entry_05_082_cache.frm
...
#if ( `$residual' != 0 )
  #message ROUNDTRIP FAILED -- dpentagon entry (5,82) cover-first cache: NumDiff has `$residual' surviving term(s)
  #terminate
#endif
#message ROUNDTRIP OK -- dpentagon entry (5,82) cover-first cache (NumDiff = 0)
print +ss;
.end
